...
**Notation.** Formulas use plain-text and Unicode mathematics. Braces or parentheses group subscripts and exponents; `binom(a,b)` is a binomial coefficient. A bar denotes an ensemble mean, a hat a numerical estimate. Logarithms are base 2 unless written `ln`. Equation numbers link directly to their displays.

<a id="sec-introduction"></a>

## 1. Introduction

At support size `M = 1`, an equal-amplitude subset state is a computational-basis product state. At full support `M = 2^n`, it is again a product state, `|+⟩^⊗n`. Between these endpoints, however, a support of size `2^(n/2)` can be arranged as a permutation matrix across a balanced cut and produce exactly maximal entanglement. This elementary contrast poses the question studied here: for a support chosen uniformly at random, how does bipartite entanglement depend on the number of occupied basis labels?
...
We first compute the ensemble-mean reduced state and the exact average purity by counting occupied rectangles in the bipartite support matrix. For a power-law support `M = cN^γ`, the resulting terms define a near-maximal window `1/2 < γ < 3/4`: `γ = 2/3` is the optimal interior balance, while `γ = 3/4` is the dense-side boundary. We then determine the finite-size von Neumann trajectory numerically. For even `n = 10, …, 30`, its broad maximum approaches the balanced Page benchmark while occupying a shrinking fraction of the basis. The exact first moment, a hypergeometric occupancy bound, and a separated-mode random-matrix ansatz explain the sparse rise and dense decline.
...
```text
ℋ^(n) = ℋ_L ⊗ ℋ_R,    dim ℋ_L = dim ℋ_R = d.
```

Unless stated otherwise, the right subsystem contains the `n/2` least significant qubits. Each computational label is then identified with a pair `x = (a,b) ∈ [d]_0 × [d]_0`. Define the binary support matrix `X_A ∈ {0,1}^(d×d)` by
...
The state and its right reduced density matrix are
...
```text
|A⟩ = ∑_(a,b=0)^(d−1) (C_A)_(ab) |a⟩_L |b⟩_R,
```
...
Numerical estimates of the peak support and peak mean entropy are denoted by `M̂_n` and `Ŝ_n`. The hats distinguish them from exact maximizers of `S̄_{N,M}`.

<a id="sec-exact-moments"></a>

## 3. Exact moments and compressed-support entanglement

Although fixing the support cardinality correlates the entries of `X_A`, the first moment of the reduced state and the average purity can be evaluated exactly. Multi-copy moment operators of fixed-size random subset states have also been analyzed in recent pseudorandomness work [[5]](#ref-5), [[4]](#ref-4). Here the balanced bipartition and support-matrix geometry lead to especially direct closed formulas for the ensemble-mean reduced state and the ensemble-average purity. For a cell `i = (a,b)`, write `X_i = 1[i ∈ A]`. If `i₁, …, i_r` are distinct cells, uniform sampling without replacement gives
...
```text
𝔼_A ρ_A^R = (1/d − β_(N,M)) I_d + β_(N,M) J_d,
β_(N,M) = (M−1) / [d(N−1)],
```
...
```text
(ρ_A^R)_(bb′) = (1/M) ∑_(a=0)^(d−1) X_(ab) X_(ab′).
```

For `b = b′`, each summand has expectation `M/N`, hence `𝔼_A(ρ_A^R)_(bb) = 1/d`. For `b ≠ b′`, the two cells are distinct and [Eq. (16)](#eq-16) gives
...
```text
𝔼_A(ρ_A^R)_(bb′) = (d/M) (M)_2 / (N)_2 = (M−1) / [d(N−1)].
```

[Equation (17)](#eq-17) follows. A matrix of the form `αI_d + βJ_d` has eigenvalue `α + dβ` on the uniform vector and `α` on its orthogonal complement. ∎

The subscript “mf” emphasizes that `λ_mf` is the uniform eigenvalue of the mean matrix, not an identity for `𝔼_A λ_max(ρ_A^R)`. At `M = 1`, for example, every individual reduced state is pure, whereas `𝔼_A ρ_A^R = I_d/d`. The quantity becomes a useful finite-size mean-field scale in the regime where sampled spectra develop an isolated leading coherent mode.
...
```text
P(A) = (1/M²) ∑_(a,a′,b,b′) X_(ab) X_(ab′) X_(a′b′) X_(a′b).
```
...
Applying [Eq. (16)](#eq-16) to these three cases gives [Eq. (23)](#eq-23). The complete count, including the small-`M` cases, is given in [Appendix C](#app-combinatorial-derivations). The endpoint checks are immediate: `P̄_{N,1} = P̄_{N,N} = 1`.

To expose the relevant powers before choosing a particular scale, let `M_N = cN^γ + O(1)` with fixed `c > 0` and `0 < γ < 1`. Expanding [Eq. (23)](#eq-23) gives
...
```text
P̄_{N,M_N} = 2N^(−1/2) + c^(−1)N^(−γ) + c²N^(2γ−2)
            + o(N^(−1/2) + N^(−γ) + N^(2γ−2)).
```
...
**Corollary 3.3 (Power-law support window and optimal balance).** Let `M_N = cN^γ + O(1)` with fixed `c > 0` and `0 < γ < 1`.
...
Within this interval, the slower of the two support-dependent corrections in [Eq. (26)](#eq-26) decays fastest at `γ = 2/3`. More precisely, for `M_N = cN^(2/3) + O(1)`,
...
```text
P̄_{N,M_N} = 2N^(−1/2) + (c^(−1) + c²) N^(−2/3) + O(N^(−1)),
```

and the coefficient `c^(−1) + c²` is minimized at `c = 2^(−1/3)`. Hence, for `M_N = 2^(−1/3)N^(2/3) + O(1)`,
...
```text
S̄_{N,M_N} ≥ n/2 − 1 − [3 / (2^(5/3) ln 2)] N^(−1/6) + O(N^(−1/3)).
```
...
```text
P̄_{N,M_N} = (2 + c²) N^(−1/2) + c^(−1)N^(−3/4) + O(N^(−1)),
```
...
```text
S̄_{N,M_N} ≥ n/2 − log₂(2 + c²) − o(1).
```

**Proof.** [Equation (26)](#eq-26) follows by inserting `M_N = cN^γ + O(1)` and `d = N^(1/2)` into [Eq. (23)](#eq-23). For `1/2 < γ < 3/4`, both support-dependent terms are `o(N^(−1/2))`. For every density matrix,
...
```text
S̄_{N,M} ≥ 𝔼_A[−log₂ P(A)] ≥ −log₂ P̄_{N,M}.
```
...
```text
S(A) ≤ log₂(rank C_A) ≤ min{log₂ M, n/2}.
```
...
Unless stated otherwise, the numerical calculations use the natural balanced cut, with the low-order `n/2` bits assigned to the right subsystem. A support is drawn uniformly without replacement from `[N]_0`. If `σ_j(C_A)` are the singular values of the coefficient matrix, then the nonzero reduced-state eigenvalues are
...
```text
S̄̂_{N,M} = (1/R) ∑ from r = 1 to R of S(A_r).
```

Error bars on sampled mean trajectories show the standard error `s_R/√R`, where `s_R` is the sample standard deviation. Figure captions state when the average is instead over balanced cuts of one fixed support.
...
```text
S_Page(n) = [ℋ_(d²) − ℋ_d − (d−1)/(2d)] / ln 2
          = n/2 − 1/(2 ln 2) + O(2^(−n)),
```

where `ℋ_k := ∑ from j = 1 to k of 1/j` [[7]](#ref-7), [[8]](#ref-8).

Peak estimates are obtained in two stages. A coarse scan first identifies a candidate maximum region. Mean entropies in a local support-size window are then fitted by a quadratic function of `x = log₂ M`; the interior maximum defines `M̂_n` and `Ŝ_n`. This procedure estimates a characteristic peak scale rather than an exact integer maximizer. The support space is combinatorial, and the trajectory is broad enough near its maximum that unit-level changes in `M̂_n` have no separate physical meaning.
...
The descending branch has a specifically coherent origin. Increasing `M` does not merely increase the number of populated basis vectors: by [Eq. (36)](#eq-36), it also increases the fidelity with `|+⟩^(⊗n)`. Near full support, this positive direction dominates and the state returns to a product state. [Section 5](#sec-spectral-mechanism) turns this observation into a spectral picture involving an isolated uniform mode and a residual bulk.

<a id="subsec-peak-scaling"></a>

### 4.3. Estimated peak and finite-size scaling

For each even `n` in the studied range, the protocol above gives an estimated peak location `M̂_n` and estimated peak mean entropy `Ŝ_n`. The resulting values are listed in [Table 1](#tab-peak-estimates). Hats are retained because the entries are numerical estimates of the ensemble peak, not exact maximizers of `S̄_{N,M}`.
...
**Table 1.** Estimated support size `M̂_n` at the maximum of the random fixed-cardinality entropy trajectory and the corresponding estimated mean entropy `Ŝ_n`. All rows are used in the finite-size fits in [Figure 2](#fig-peak-scaling).
...
**Figure 2.** Estimated peak support size and peak entropy for even `n = 10,…,30`. Dashed lines are the least-squares finite-size fits to `log₂ M̂_n` and `Ŝ_n` in [Eq. (41)](#eq-41); the dotted curve is the exact Page mean for a balanced bipartition. The common vertical coordinate reports base-2 values: `Ŝ_n` and the Page mean are entropies in bits, whereas `log₂ M̂_n` is the base-2 logarithm of the support size. The fits summarize the studied range and are not asserted as asymptotic laws.

Two finite-size observations are robust at the scale of the data. First, the peak entropy tracks the Page mean increasingly closely: the difference `S_Page(n) − Ŝ_n` decreases from about `0.21` bits at `n=10` to about `0.016` bits at `n=30`. Second, the estimated peak occupies a shrinking fraction of the computational basis. The ratio `M̂_n/N` decreases from approximately `10.4%` at `n=10` to `0.171%` at `n=30`, while the second fit in [Eq. (41)](#eq-41) summarizes its exponential growth in absolute size.
...
Across the retained rows it increases monotonically from approximately `0.674` at `n=10` to `0.694` at `n=30`. Both these ratios and the fitted slope lie between the correction-balanced interior value `2/3` and the dense-side boundary `3/4` identified in [Corollary 3.3](#cor-compressed-support-entropy). Their upward movement over the studied range is compatible with an eventual relation to the `3/4` boundary, but it does not determine an asymptotic exponent: a finite intercept and other subleading corrections can produce the same behavior. The purity scales and the finite-size location of the von Neumann maximum remain different quantities and need not coincide.

The empirical conclusion is nevertheless strong: equal-positive-amplitude states can attain near-Page balanced entanglement while occupying an exponentially compressed support. This entanglement statement is logically distinct from pseudorandomness. The scale `M ≍ N^(2/3)` from [Corollary 3.3](#cor-compressed-support-entropy) lies within fixed-size regimes where polynomial-copy moment averages of random subset states are known to approach their Haar counterparts [[5]](#ref-5), [[4]](#ref-4). The present work isolates the balanced-entanglement consequence and its support-size trajectory; it does not rederive those indistinguishability results.
...
**Figure 3.** Reduced-state spectrum for one random subset state at `n=24` and `M=93018`, near the estimated entropy peak. The histogram contains the bulk eigenvalues after removing the largest eigenvalue `λ₀ := λ_max(ρ_A^R)`. The inset compares the observed `λ₀` with the exact uniform eigenvalue `λ_mf` of the ensemble-mean reduced state and with the leading approximation `M/N`. The mean-matrix mode is a finite-size mean-field scale, not an exact formula for the largest eigenvalue of each sample.

To model the dense side, we use `λ_mf` as the separated mode and approximate the remaining trace by a balanced random-matrix bulk. If the normalized residual bulk has the Page entropy `log₂ d − 1/(2 ln 2)`, then the entropy decomposes into the binary mixing entropy of the separated mode and the rescaled bulk entropy. Writing `h₂(x) := −x log₂ x − (1−x) log₂(1−x)`, this gives the fixed-mode ansatz
...
```text
T_{N,M} = h₂(λ_mf) + (1−λ_mf)[log₂ d − 1/(2 ln 2)],
```

with the usual endpoint convention and `λ_mf = λ_mf(N,M)` from [Eq. (18)](#eq-18). This is a dense-bulk ansatz, not an exact moment formula. It uses the exact first moment of the fixed-cardinality ensemble but still assumes that the residual spectrum behaves like a normalized random bulk [[7]](#ref-7), [[26]](#ref-26).
...
In the dense range `M ≫ d`, this is asymptotic to `M/N`. For `M = cN^γ`, its squared weight therefore scales as `c²N^(2γ−2)`, the same power as the rectangle term in [Eq. (26)](#eq-26). At `γ = 3/4` this contribution is of order `N^(−1/2)` and becomes comparable with the balanced random-matrix background at the level of the second moment. This identifies the upper boundary without asserting that the observed largest eigenvalue equals the mean-matrix mode in every sample.
...
```text
W_b := ∑ from a = 0 to d−1 of X_ab
```

be the number of occupied cells in column `b`. Because the support contains exactly `M` cells sampled from the `N = d²` available cells, `W_b` is hypergeometric rather than binomial:
...
```text
Pr(W_b = w) = binom(d,w) binom(N−d,M−w) / binom(N,M),
```
...
```text
max(0, M−N+d) ≤ w ≤ min(d,M).
```

Since `(ρ_A^R)_bb = W_b/M`, exchangeability of the columns gives the exact mean diagonal entropy
...
```text
D_{N,M} := −d ∑ over w of Pr(W_b = w) (w/M) log₂(w/M),
```
...
Low-order residue classes have a direct entanglement consequence for the natural balanced cut. Let `ℓ := n/2`, choose `0 ≤ t ≤ ℓ`, and partition a support `A` according to its residues modulo `2^t`:
...
```text
M_t(B) := (|B_0^(t)|, …, |B_(2^t−1)^(t)|)
```

and let `ℰ_t(B)` be the uniform ensemble of supports `A` satisfying
...
```text
μ_t^F(B) := E over A ∼ ℰ_t(B) of S_F(A),
Δ_t^F(B) := μ_t^F(B) − S_F(B).
```
...
Random equal-positive-amplitude subset states interpolate between product states at single and full support and a broad intermediate regime of high balanced entanglement. For the uniform fixed-cardinality ensemble, we obtained the exact mean reduced state and average purity. If `M = cN^γ`, every fixed `1/2 < γ < 3/4` gives `S̄_{N,M} ≥ n/2 − 1 − o(1)`; `γ = 2/3` uniquely balances the sparse and rectangle corrections, while `γ = 3/4` marks the dense-side boundary. This is a support-size guarantee, not a theorem locating the von Neumann entropy maximum.

The numerical trajectory for even `n = 10,…,30` rises from zero, approaches the balanced Page mean near its maximum, and returns to zero at full support. The peak support occupies a decreasing basis fraction, while its effective exponent increases over the studied range and remains between `2/3` and `3/4`. The hypergeometric diagonal-entropy bound and separated-mode ansatz explain the opposite sides of the curve: sparse supports gain Schmidt sectors, whereas dense positive supports reinforce the uniform coherent mode.
...
We write `S̄^(α)_{N,M}` for the fixed-cardinality ensemble mean. For every individual reduced state,
...
Let `A` be uniform among all `M`-element subsets of `[N]₀`, and let `X_i = 1[i ∈ A]`. For `r` distinct cells `i₁, …, i_r`, the event `i₁, …, i_r ∈ A` leaves `M − r` support elements to be chosen from the remaining `N − r` cells. Hence
...
```text
E_A[∏_{j=1}^r X_{i_j}] = Pr(i₁, …, i_r ∈ A)
                       = binom(N − r, M − r) / binom(N, M)
                       = (M)_r / (N)_r.
```
...
```text
Tr[(ρ_A^R)²] = ∑_{b,b′} (ρ_A^R)_{bb′} (ρ_A^R)_{b′b}
             = (1/M²) ∑_{a,a′,b,b′} X_{ab} X_{ab′} X_{a′b′} X_{a′b}.
```
...
Let `M = cN^γ + O(1)` with fixed `c > 0`, `0 < γ < 1`, and `d = N^(1/2)`. The three terms in [Eq. (23)](#eq-23) satisfy
...
```text
P̄_{N,M} = 2N^(−1/2) + c^(−1) N^(−γ) + c²N^(2γ−2)
           + O(N^(−1) + N^(−2γ) + N^(−γ−1/2)
               + N^(2γ−5/2) + N^(γ−2)),
```
...
[Equation (76)](#eq-76) gives the three regimes in [Corollary 3.3](#cor-compressed-support-entropy): at `γ = 1/2` the sparse term joins the background, throughout `1/2 < γ < 3/4` both support-dependent terms are `o(N^(−1/2))`, and at `γ = 3/4` the rectangle term joins the background. Within the interior window, the slower correction is governed by `min{γ, 2 − 2γ}` and is optimized by
...
At this exponent the remaining coefficient is `c^(−1) + c²`; the condition `−c^(−2) + 2c = 0` gives `c = 2^(−1/3)`. Substitution into [Eq. (76)](#eq-76), followed by [Eq. (34)](#eq-34) and `log₂(1 + x) = x/ln 2 + O(x²)`, yields [Eqs. (29)](#eq-29)–[(30)](#eq-30).
...
```text
E_A H(diag ρ_A^R)
    = −∑_{b=0}^{d−1} E_A[(W_b/M) log₂(W_b/M)]
    = −d ∑_w Pr(W_b = w) (w/M) log₂(w/M)
    = D_{N,M}.
```
...
```text
S(ρ_A^R) ≤ S(Δ(ρ_A^R)) = H(diag ρ_A^R),
```
...
```text
ℋ_R = ℋ_{R′} ⊗ ℋ_Q,   dim ℋ_Q = 2^t,   dim ℋ_{R′} = 2^(ℓ−t),
```

where `Q` contains the `t` least significant qubits. For every occupied residue `r`, strip those bits from the labels in `A_r^(t)` and define the normalized branch state `|ψ_r⟩ ∈ ℋ_L ⊗ ℋ_{R′}`. Then
...
```text
|A⟩ = ∑_{r: p_r^(t)>0} √(p_r^(t)) |ψ_r⟩ |r⟩_Q.
```

Let `ρ_R = Tr_L |A⟩⟨A|` and dephase `Q` in its computational basis. The dephasing channel is a uniform mixture of commuting phase unitaries, so concavity and unitary invariance of the von Neumann entropy imply
...
```text
S(𝒟_Q(ρ_R)) = H(p^(t)) + ∑_r p_r^(t) S(ρ_{R′}^(r))
```
...
because each `ρ_{R′}^(r)` acts on `2^(ℓ−t)` dimensions. This proves [Eq. (52)](#eq-52) in [Proposition 6.1](#prop-residue-ceiling). If only `q` residues are occupied, `H(p^(t)) ≤ log₂ q`, proving [Eq. (53)](#eq-53). For a single occupied residue, [Eq. (82)](#eq-82) contains one term, so the `t` qubits in `Q` factor exactly from the remaining state.

### D.2. Conditional support sampler

For a deterministic support `B`, the ensemble `ℰ_t(B)` is sampled by partitioning `[N]₀` into the `2^t` residue classes and choosing `M_r^(t)(B)` labels uniformly without replacement from class `r`, independently across classes. This produces the uniform distribution over all supports with the prescribed count vector because every admissible support has probability
...
```text
∏_{r=0}^{2^t−1} binom(N/2^t, M_r^(t)(B))^(−1).
```
