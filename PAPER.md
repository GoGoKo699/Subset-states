...
```math
S(A):=-\operatorname{Tr}\!\left(\rho_A^R\log_2\rho_A^R\right).
\tag{13}
```
...
```math
P(A):=\operatorname{Tr}\!\left[(\rho_A^R)^2\right],\qquad\overline{P}_{N,M}:=\mathbb{E}_A P(A).
\tag{22}
```
...
```math
S(A)\leq\log_2\!\bigl(\operatorname{rank}C_A\bigr)\leq\min\!\left\{\log_2 M,\frac{n}{2}\right\}.
\tag{35}
```
...
S(A)
    ...
    =H\!\left(\operatorname{diag}\rho_A^R\right).
\tag{49}
```
...
```math
S^{(\alpha)}(A)
:=\frac{\log_2\operatorname{Tr}\!\left[(\rho_A^R)^\alpha\right]}{1-\alpha},
\tag{63}
```
...
```math
\begin{aligned}
\operatorname{Tr}\!\left[(\rho_A^R)^2\right]
...
```math
W_b\sim\operatorname{Hypergeometric}(N,d,M),
\tag{78}
```
...
```math
\begin{aligned}
\mathbb E_A H(\operatorname{diag}\rho_A^R)
...
Let $\rho_R=\operatorname{Tr}_L\lvert A\rangle\langle A\rvert$ and dephase $Q$ in its computational basis. The dephasing channel is a uniform mixture of commuting phase unitaries, so concavity and unitary invariance of the von Neumann entropy imply
...
```math
\rho_{R'}^{(r)}
:=\operatorname{Tr}_L\lvert\psi_r\rangle\langle\psi_r\rvert,
\tag{84}
```
