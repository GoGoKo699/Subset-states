...
The repository check verifies frozen evidence hashes, local Markdown link paths, and the absence of TeX/PDF artifacts. It does not fetch external links or inspect missing historical records.
...
This samples a neighborhood around a supplied Table I value, then fits a quadratic in log₂ M. It does not search the entire support-size range. The original global searches for the historical peaks are not part of `--full` either.
